# Story EVAL-007: KoBALT-700 Adapter
...
Implement a KoBALT-700 benchmark adapter that loads versioned local input files through explicit field mapping, normalizes supported choice-answer rows into the internal benchmark schema, reuses the common MCQ prompt/parser/scoring primitives where applicable, and preserves invalid or failed rows with clear failure categories.

## Context

Depends on EVAL-001, EVAL-003, EVAL-005, and EVAL-006.
...
- [x] KoBALT-700 adapter package/module exists under `harness/benchmarks/`.
...
- [x] Adapter does not infer, rename, or fabricate missing KoBALT-700 dataset fields.
...
- [x] Offline fixture tests cover valid mapped rows, explicit custom mapping, missing required mapped fields, unsupported row shape, invalid answer labels, and aggregate scoring.

## Out of Scope

- Downloading or redistributing KoBALT-700 data
- Assuming official KoBALT-700 source field names or task layout
- Hardcoding real model tags
- Running full benchmark inference against Ollama models
- Implementing CLIcK or IFEval-Ko adapters
- Result persistence and Markdown report generation
- MileDay-specific schedule generation evaluation

## Expected Files

- `harness/benchmarks/kobalt_700.py`
- `tests/harness/benchmarks/test_kobalt_700.py`
- `tests/fixtures/benchmarks/kobalt_700/`
- Optional small local fixture files under `tests/fixtures/benchmarks/kobalt_700/`

## Implementation Notes

- Treat KoBALT-700 input files as user-provided/versioned local artifacts.
- Keep fixture data synthetic and minimal; do not claim it is official KoBALT-700 data.
...
- Reuse the EVAL-006 adapter pattern where it applies, but do not share code by hiding dataset-specific behavior behind unclear abstractions.
