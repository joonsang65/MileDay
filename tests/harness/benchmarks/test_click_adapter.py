...
from harness.benchmarks.click_adapter import (
    BENCHMARK_ID,
    CLIcKDatasetError,
    CLIcKFieldMapping,
    load_click_cases,
    score_click_responses,
)
from harness.schemas import FailureCategory


FIXTURE_PATH = Path("tests/fixtures/benchmarks/click/synthetic.jsonl")


def test_loads_synthetic_fixture_into_internal_benchmark_cases():
    cases = load_click_cases(
        FIXTURE_PATH,
        dataset_id="click-synthetic-v1",
        mapping=CLIcKFieldMapping(metadata={"source_version": "source_version"}),
    )

    assert len(cases) == 2
    assert cases[0].benchmark_id == BENCHMARK_ID
    assert cases[0].dataset_id == "click-synthetic-v1"
    assert cases[0].case_id == "synthetic-1"
    assert cases[0].category == "reading"
    assert cases[0].question == "Which option best completes the sentence? The sky is ____."
...
def test_build_prompt_reuses_common_mcq_prompt_builder():
    cases = load_click_cases(FIXTURE_PATH)
...
def test_missing_dataset_file_is_dataset_unavailable():
    with pytest.raises(CLIcKDatasetError) as exc_info:
        load_click_cases("tests/fixtures/benchmarks/click/missing.jsonl")
...
def test_custom_mapping_is_explicit_and_does_not_infer_fields(tmp_path):
    ...
    cases = load_click_cases(source, mapping=mapping)
...
def test_missing_mapped_required_field_is_schema_changed(tmp_path):
    ...
    with pytest.raises(CLIcKDatasetError) as exc_info:
        load_click_cases(
            source,
            mapping=CLIcKFieldMapping(choices={"A": "choice_a", "B": "choice_b"}),
        )
...
def test_unsupported_non_choice_answer_row_is_schema_changed(tmp_path):
    ...
    source.write_text(
        '{"case_id":"case-1","question":"Write a free-form explanation.","answer":"open"}\n',
        encoding="utf-8",
    )

    with pytest.raises(CLIcKDatasetError) as exc_info:
        load_click_cases(source)
...
def test_aggregate_scoring_reuses_common_mcq_scoring():
    ...
    assert report.by_category["reading"].invalid == 1
