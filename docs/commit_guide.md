...
FastAPI 기반 인증 API 명세 작성을 위해 auth 라우터를 추가한다.
회원가입, 로그인, 로그아웃, 현재 사용자 조회 endpoint의 기본 응답 구조를 정의한다.
```
...
```txt
API 명세서 작성을 위해 백엔드 라우터의 기본 골격을 추가한다.
...
캘린더 화면에서 오늘 수행할 마일스톤을 조회하기 위한 API 골격을 추가한다.
/calendar/today endpoint를 등록하고 기본 응답 구조를 정의한다.
```
...
프론트엔드에서 기대하는 token_type 필드와 백엔드 응답명이 달라 인증 상태 저장이 실패했다.
로그인 응답 스키마의 필드명을 API 명세서 기준에 맞게 수정한다.
```
...
기능별 API 라우터를 명확히 구분하기 위해 main.py의 라우터 등록 코드를 정리한다.
...
로컬 FastAPI 실행을 위해 개발 환경 변수 예시를 추가한다.
.env.example에 Supabase 연결 정보와 로그 레벨 설정 항목을 정의한다.
```
...
Supabase PostgreSQL 기반의 MileDay 핵심 데이터 구조를 문서화한다.
goals, milestones, user_settings, external_calendar_connections 테이블의 역할과 필드를 정리한다.
```
...
프론트엔드와 백엔드 간 통신 규칙을 정의하기 위해 API 명세서 초안을 작성한다.
공통 응답 형식, 인증 방식, MVP endpoint 목록과 요청/응답 예시를 정리한다.
```
