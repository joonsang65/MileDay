...
## 17. 사용자 설정과 공휴일 API 연동

### 증상

사용자 설정 API가 mock 응답이면 설정을 저장해도 앱 재실행 후 기본 캘린더 view, 주 시작 요일, 언어 설정이 유지되지 않는다.

공휴일 API 키가 없거나 외부 API 호출이 실패하면 공휴일 이름이 표시되지 않을 수 있다.

### 원인

기존 `/settings` 라우터는 인증과 `user_settings` 저장소를 사용하지 않는 임시 병합 응답이었다.

공공데이터포털 공휴일 API는 별도 서비스 키가 필요하므로 로컬 개발 환경에서 키가 없을 수 있다.

### 처리 기준

`/settings`는 JWT의 현재 사용자 ID로 `user_settings` row를 조회하고, 없으면 기본값 row를 생성한다.

공휴일 API 키가 없거나 API 호출이 실패해도 캘린더 조회는 실패하지 않는다.

fallback은 공휴일 목록을 비워 두고, 프론트에서 토요일/일요일 주말 표시만 유지하는 방식으로 처리한다.

### 확인 방법

- `HOLIDAY_API_SERVICE_KEY`가 설정되어 있는지 확인한다.
- 한국천문연구원 특일 정보 API 활용 신청이 해당 서비스 키에 승인되어 있는지 확인한다.
- 실제 호출이 `https://apis.data.go.kr/B090041/openapi/service/SpcdeInfoService/getHoliDeInfo`로 나가는지 확인한다.
- 설정 변경 후 `GET /settings`에서 저장된 값이 반환되는지 확인한다.
- `week_starts_on`을 `0`으로 저장하면 캘린더 요일 라벨이 일요일부터 시작하는지 확인한다.
- `language`를 `en`으로 저장하면 헤더와 설정 패널의 핵심 문구가 영어로 바뀌는지 확인한다.

### 한국천문연구원 API 403

특일 정보 API URL과 파라미터가 맞아도 공공데이터포털에서 `403`을 반환할 수 있다.

이 경우 코드의 fallback 때문에 앱은 정상 동작하지만 공휴일 목록은 비어 있다.
