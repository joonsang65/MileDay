...
- 한글 문서, 주석, 로그, 커밋 메시지, 코드 문자열은 UTF-8을 기준으로 읽고 작성함
- PowerShell에서 한글이 깨지거나 `Get-Content` 결과가 비정상적으로 보이면 `Get-Content -Encoding UTF8`로 다시 확인함
- 기존 파일 수정 중 깨진 한글 문자열이 보이면 원문 의미를 확인한 뒤 UTF-8 기준으로 함께 정리함
- 새 Markdown 문서와 코드 파일은 UTF-8로 저장하고, 인코딩 문제를 피하기 위해 임의의 ANSI/CP949 변환을 하지 않음
- 인코딩 오류가 의심되는 상태에서 깨진 문자열을 그대로 근거로 삼아 구현하거나 문서화하지 않음
