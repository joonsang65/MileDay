...
BENCHMARK_ID = "click"
DEFAULT_DATASET_ID = "click-local"


class CLIcKDatasetError(ValueError):
...
class CLIcKFieldMapping(BaseModel):
...
class CLIcKCase(BaseModel):
    ...
    def _validate_answer_membership(self) -> CLIcKCase:
...
def load_click_cases(
    source_path: str | Path,
    *,
    dataset_id: str = DEFAULT_DATASET_ID,
    mapping: CLIcKFieldMapping | None = None,
) -> list[CLIcKCase]:
    path = Path(source_path)
    field_mapping = mapping or CLIcKFieldMapping()
...
def score_click_responses(
    cases: Iterable[CLIcKCase],
...
def _load_rows(path: Path) -> list[dict[str, Any]]:
    if not path.exists() or not path.is_file():
        raise CLIcKDatasetError(
            FailureCategory.DATASET_UNAVAILABLE,
            f"CLIcK dataset file does not exist or is not readable: {path}",
        )
    ...
    raise CLIcKDatasetError(
        FailureCategory.DATASET_UNAVAILABLE,
        f"Unsupported CLIcK dataset file extension: {path.suffix}",
    )
...
def _normalize_row(
    ...
    *,
    ...
    try:
        ...
        raise CLIcKDatasetError(
            FailureCategory.DATASET_SCHEMA_CHANGED,
            f"Invalid CLIcK row {row_number}: {exc}",
        ) from exc


def _reject_unsupported_shape(
    row: dict[str, Any], *, mapping: CLIcKFieldMapping, row_number: int
) -> None:
...
def _required_text(row: dict[str, Any], field_name: str, row_number: int) -> str:
    ...
    if value is None or str(value).strip() == "":
        raise CLIcKDatasetError(
            FailureCategory.DATASET_SCHEMA_CHANGED,
            f"CLIcK row {row_number} has blank mapped field '{field_name}'.",
        )
